...
class HomeCategotyAdapter (
    ...
    inner class MyViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        val parentLayout = itemView.findViewById<LinearLayout>(R.id.category_itemview)
        var ivProductPhoto = itemView.findViewById<ImageView>(R.id.img_Category)
        var tvProductName = itemView.findViewById<TextView>(R.id.tvCategoryName)
...
//            tvBtnAddToCart.setCompoundDrawablesWithIntrinsicBounds(
...
//            if (getItem(position).main_price?.toDouble()==getItem(position).selling_price?.toDouble()){
...
//            holder.tvMainPrice.text =
//                getItem(position).main_price + " " + getItem(position).currency
...
//            }
        ...
        /*     Glide.with(context).load(getItem(position).main_image).thumbnail(0.1f)
             ...
             holder.ratingBar.rating = getItem(position).rating!!.toFloat()
     //
     //        Log.d("list==",getItem(position).name+"khan")*/
